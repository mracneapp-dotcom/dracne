// app/MyDayRoutine.js - WITH ROUTINE LEVEL INDICATOR
...
export default function MyDayRoutine({ 
  ...
  const [routineLevel, setRoutineLevel] = useState(null); // 'basic', 'moderate', or 'intensive'
  ...
  const loadRoutineData = async () => {
    try {
      ...
      // Try to load from both basic and moderate routines
      ...
      // Determine which routine to display (prioritize most recent)
      ...
      if (basicRoutine) {
        ...
        // If no moderate routine, or basic is more recent, use basic
        if (!selectedRoutine || 
        ...
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              ...
              console.log(`🗑️ ${levelText} routine cleared`);
            } catch (error) {
  ...
  const renderRoutineInfo = () => {
    ...
    const stepCount = routineLevel === 'moderate' ? 4 : 3;
  ...
  return (
    <View style={styles.container}>
      ...
      {/* Banner - Navigable to DayRoutine */}
      <TouchableOpacity 
      ...
      >
        <View style={styles.content}>
          {/* Badges Row - Skin Type + Routine Level */}
          ...
          {loading ? (
            ...
            <>
              ...
              {routineLevel === 'moderate' && renderRoutineStep(
                ...
                STEP_ICONS.moisturizer, // You can add a new icon if you have one
                routineData.specializedProducts
              )}

              {/* Step 3/4: Sunscreen */}
              {renderRoutineStep(
                routineLevel === 'moderate' ? 4 : 3,
                'Sunscreen (SPF 30+)',
              ...
              <View style={styles.footerBox}>
                ...
                <Text style={styles.footerText}>
                  • Apply products in order: Cleanser → Moisturizer{routineLevel === 'moderate' ? ' → Specialized Treatment' : ''} → Sunscreen{'\n'}
